...
size_label = 35;
stride = 35;
 data1 = zeros(size_input, size_input, 1, 56000);%note : re-caculate the last value accroding to your dataset size
 data2 = zeros(size_input, size_input, 1, 56000);
 label = zeros(size_label, size_label, 1, 56000);
 padding = abs(size_input - size_label)/2;
 test_num = 100;

for QP = [22 27 32 37]
    folder_data = ['../../dataset/rec_yuvQ',num2str(QP),'_DF'];
    folder_label = '../../dataset/oriyuv';
    savepath = ['test_data_Y_','withQP_DF.h5'];
...
%                     subim_input_V = ImgData_V(x : x+size_input-1, y : y+size_input-1);
                     subim_label_Y = Imglabel_Y(x+padding : x+padding+size_label-1, y+padding : y+padding+size_label-1);
...
%                     subim_label_V = Imglabel_V(x+padding : x+padding+size_label-1, y+padding : y+padding+size_label-1);
                    count=count+1;
                    data1(:, :, 1, count) = QPvalue;
                    data2(:, :, 1, count) = subim_input_Y;
                    label(:, :, 1, count) = subim_label_Y;
                end
            end
            fclose(fid);
            fclose(fid_label);
...
for batchno = 1:floor(count/chunksz)
    ...
    curr_dat_sz = store2hdf5_chroma(savepath, batchdata1, batchdata2,batchlabs, ~created_flag, startloc, chunksz);
    created_flag = true;
